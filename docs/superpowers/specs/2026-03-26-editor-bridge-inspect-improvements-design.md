...
refresh の `send_action` 呼び出しは fire-and-forget とする。失敗してもスクリーンショット撮影は続行する（refresh はベストエフォートの前処理であり、失敗で撮影を中断する意味がない）。refresh の成否はログに記録するが、MCP レスポンスには含めない。
...
`diagnostics` は `list[str]` で、人間可読な理由文を格納する（例: `"No renderer blocks found in base prefab or nested prefabs"`）。MCP ツール `inspect_materials` のレスポンス JSON には `diagnostics` キーとして含める。空の場合はキーを省略する（後方互換のため）。`format_materials()` は `diagnostics` が非空の場合、末尾に理由を追記する。
