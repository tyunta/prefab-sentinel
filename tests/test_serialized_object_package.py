"""Structural tests guarding the serialized_object package surface (issue #91).

This file pins the package's public-surface invariant only; the line-count
invariant for files under the package is enforced by the CI-side static
...
class SetComponentFieldsSER003Tests(unittest.TestCase):
    ...
    def test_set_properties_returns_ser003_for_unknown_property(self) -> None:
        ...
        # Issue #304: unified wire shape — the category lives in
        # ``code`` instead of the legacy ``detail`` slot.
