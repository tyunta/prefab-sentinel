...
class TestStatus(unittest.TestCase):
        ...
        self.assertEqual(
            {
                ...
                "script_map_size": 0,
                "script_map_cached": False,
                "symbol_tree_entries": 0,
                "symbol_tree_paths": [],
    ...
    def test_after_warm(self, mock_build: MagicMock, mock_orch: MagicMock) -> None:
        ...
        s = session.status()
        # Pin both cache flags + the warm size as one tuple so a mutation
        # to any single field surfaces against the whole post-warm
        # signature.
        self.assertEqual(
            (True, True, 2),
            (s["orchestrator_cached"], s["script_map_cached"], s["script_map_size"]),
        )
...
class TestActivate(unittest.TestCase):
    ...
    def test_activate_sets_scope_and_pins_both_cache_flags_to_true(
        self,
        mock_find: MagicMock,
        ...
        # Pin (project_root, scope, orchestrator_cached, script_map_cached)
        # as a single tuple so a mutation that drops either cache flag is
        # named in the same failure message.
        self.assertEqual(
            (
                Path("/unity"),
                Path("/unity/Assets/MyScope"),
                True,
                True,
            ),
            (
                ...
                session.scope,
                result["orchestrator_cached"],
                result["script_map_cached"],
            ),
        )
    ...
    def test_activate_explicit_project_root_overrides_existing(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                result["orchestrator_cached"],
                msg=f"explicit project_root override must warm the orchestrator cache; got {result!r}",
            )
    ...
    def test_activate_with_bogus_env_path_raises_typed_error_naming_path(
        self,
    ) -> None:
        # Tmp directory has no ``Assets`` subtree, so the project-root
        # detector's documented "not a Unity project root" path triggers.
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {UNITY_PROJECT_PATH_ENV: tmpdir}):
                session = ProjectSession()
        ...
        # assertRaises already pins the type; pin the documented failure
        # phrase too so a mutation that drops "Invalid Unity project root"
        # from the message (or swaps the typed error for one carrying an
        # empty / unrelated message) is killed here.  The phrase is the
        # domain invariant of ``InvalidProjectRootError`` per session.py.
        message = str(cm.exception)
        self.assertIn(
            "Invalid Unity project root",
            message,
            msg=f"error must name the documented root error phrase; got {message!r}",
        )

    def test_activate_with_bogus_explicit_root_raises_typed_error_naming_path(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            session = ProjectSession()
    ...
    @patch("prefab_sentinel.session.resolve_scope_path")
    @patch("prefab_sentinel.session.find_project_root")
    def test_activate_consults_project_root_detector_once_when_no_root(
        self,
        mock_find: MagicMock,
        mock_resolve: MagicMock,
        mock_orch: MagicMock,
        ...
        # Documented contract: with no env var and no explicit root, the
        # project-root detector is consulted exactly once and its return
        # is adopted as the session root.
        documented_root = Path("/unity")
        mock_find.return_value = documented_root
        mock_resolve.return_value = documented_root / "Assets" / "Scope"
        mock_build.return_value = {}
        ...
        # Pin (detector call count, adopted root) together so a mutation
        # that drops the call OR adopts a different root is named here.
        self.assertEqual(
            (1, documented_root),
            (mock_find.call_count, session.project_root),
        )
...
class TestWatcherDoneCallback(unittest.TestCase):
    ...
    def test_crashed_watcher_logs_error_message_carrying_exception_text(
        self,
    ) -> None:
        session = ProjectSession()
        ...
        # Materialise the matching log lines and pin set membership; an
        # any-predicate that loses the matching message identity would
        # leave the failure message vague.
        matching = [msg for msg in cm.output if "boom" in msg]
        self.assertEqual(
            1,
            len(matching),
            msg=(
                f"exactly one ERROR-level log carrying 'boom' expected; "
                f"all output={cm.output!r}"
            ),
        )
