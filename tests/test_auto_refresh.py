...
from prefab_sentinel.editor_bridge import BRIDGE_WATCH_DIR_ENV, bridge_status
...
class TestBridgeStatus(unittest.TestCase):
    ...
    def test_not_connected_when_watch_dir_missing(self) -> None:
        with patch.dict(os.environ, {BRIDGE_WATCH_DIR_ENV: ""}, clear=False):
            status = bridge_status()
            self.assertFalse(status["connected"])

    def test_not_connected_when_watch_dir_does_not_exist(self) -> None:
        with patch.dict(
            os.environ,
