...
`prefab-sentinel report smoke-history` は applyアサーション情報（`expected_applied` / `expected_applied_source` / `actual_applied` / `applied_matches`）をCSV出力に含め、Markdownには target別の `applied_mismatches` / `applied_pass_pct` と `observed_timeout_breaches` / `observed_timeout_coverage_pct` を表示する。
