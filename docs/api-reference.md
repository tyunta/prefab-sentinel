...
Issue #72 の live Unity 確認は source-level tests 後に `deploy_bridge` で Bridge C# を配置し、Unity compile error 0 件、foreground true-timeout、background deferred timeout、async submit/poll retention を実 Unity project で確認する。Source tests は Bridge の field / builder / callsite wiring を固定するが、Unity version ごとの focus/reload runtime 差はこの live 確認で補完する。
