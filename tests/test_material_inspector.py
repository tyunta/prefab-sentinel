...
class TestNestedRecursiveTraversal(unittest.TestCase):
    ...
    def test_3_level_deep_nesting(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(["Assets/Leaf.prefab"], source_prefabs)

    def _run_mixed_direct_and_nested_scenario(self, *, wrap_in_variant: bool) -> None:
        """Shared fixture for the (direct renderer + PrefabInstance→Child
        with renderer) → 2 renderers expectation.

        Issue #222 Phase 2: the only delta between the previous
        ``test_mixed_direct_and_nested_renderers`` and
        ``test_variant_mixed_direct_and_nested_renderers`` was whether
        the inspected target was the base prefab directly or a Variant
        of it; parametrising the scenario removes the duplication.
        """
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            assets_dir = tmp_path / "Assets"
            ...
            if wrap_in_variant:
                ...
                inspected = assets_dir / "Variant.prefab"
                inspected.write_text(variant_text, encoding="utf-8")
            else:
                inspected = base_path

            result = inspect_materials(str(inspected), project_root=tmp_path)
            self.assertEqual(
                2,
                len(result.renderers),
                f"Expected 2 renderers (direct + nested), got {len(result.renderers)}",
            )

    def test_mixed_direct_and_nested_renderers_per_wrap(self) -> None:
        for label, wrap_in_variant in (
            ("base_prefab", False),
            ("wrapped_in_variant", True),
        ):
            with self.subTest(label=label):
                self._run_mixed_direct_and_nested_scenario(
                    wrap_in_variant=wrap_in_variant,
                )
...
class TestMaterialInspectorSourceTextInvariants(unittest.TestCase):
    """Issue #222 — source-grep assertions on ``material_inspector.py``
    that read the file text but cannot kill mutmut mutations.

    The single test pins the absence of the legacy
    ``SOURCE_PREFAB_PATTERN`` identifier (mutmut cannot introduce a new
    identifier, so no mutation in the surface can flip this assertion).
    Marked ``source_text_invariant`` so the meta-mutation runner skips
    it via ``-m "not source_text_invariant"`` rather than logging a
    misleading "test cannot kill any mutant" entry; sibling behavioural
    classes in this module remain unmarked and continue to contribute
    mutant-detection signal.
    """
