...
# Issue #295: Unity's class ID for the Material asset block.
_CLASS_ID_MATERIAL = "21"


def _build_material_symbol_tree(
    text: str, source_path: str,
) -> SymbolTree:
    """Construct the Material-asset branch of the symbol tree (issue #295).

    The synthetic root names the asset stem (``Path(source_path).stem``)
    and carries one property-kind leaf per serialized material property
    (``m_Floats`` / ``m_Ints`` / ``m_Colors`` / ``m_TexEnvs``). The
    addressing scheme on ``find_unity_symbol`` is therefore
    ``<asset-stem>/<property-name>``.
    """
    property_names = list_material_property_names(text)
    if not property_names:
        # Degenerate input: Material block present but no serialized
        # properties.  Return an empty tree without raising so callers
        # see the existing degenerate-input behaviour.
        return SymbolTree(asset_path=source_path)

    asset_stem = Path(source_path).stem if source_path else ""
    if not asset_stem:
        asset_stem = "Material"

    leaves: list[SymbolNode] = []
    ...
    root = SymbolNode(
        ...
        name=asset_stem,
        file_id="",
        class_id=_CLASS_ID_MATERIAL,
        children=leaves,
        depth=0,
    )
    return SymbolTree(asset_path=source_path, roots=[root])
...
def build_symbol_tree(
    ...
    *,
    ...
    # Issue #295: Material assets carry no GameObject blocks but expose
    # serialized properties under ``m_SavedProperties``. The builder
    # branches on input asset shape so the GameObject-driven path is
    # preserved for .prefab / .unity inputs.
    if not game_objects and any(
        block.class_id == _CLASS_ID_MATERIAL for block in blocks
    ):
        return _build_material_symbol_tree(text, source_path)
