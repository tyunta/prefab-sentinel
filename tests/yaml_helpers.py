...
def make_transform(
    ...
    *,
    ...
    size_delta: tuple[float, float] = (100.0, 100.0),
    pivot: tuple[float, float] = (0.5, 0.5),
) -> str:
    """Build a Transform / RectTransform YAML block.

    ``anchor_min`` / ``anchor_max`` / ``anchored_position`` / ``size_delta``
    / ``pivot`` are honoured only when ``is_rect`` is ``True``; plain
    Transform blocks omit those fields entirely. The defaults reproduce
    Unity's default centred-pivot rect (issue #238).
    """
    ...
    if is_rect:
        rect_block = (
            ...
            f"  m_AnchoredPosition: {{x: {anchored_position[0]}, y: {anchored_position[1]}}}\n"
