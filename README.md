...
uvx --from . prefab-sentinel report smoke-history --inputs "reports/bridge_smoke/**/summary.json" --out "reports/bridge_smoke_history.csv" --max-applied-mismatches 0 --min-applied-pass-pct 100
...
`prefab-sentinel report smoke-history` は applyアサーション情報（`expected_applied` / `expected_applied_source` / `actual_applied` / `applied_matches`）をCSV出力に含め、Markdownにも target別の `applied_mismatches` / `applied_pass_pct` を表示する。
`prefab-sentinel report smoke-history --max-applied-mismatches N --min-applied-pass-pct P` は applyアサーション品質ゲートを有効化し、閾値違反時はCSV/Markdownを出力したうえで exit code 1 を返す。
