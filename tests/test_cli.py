...
""".strip(),
    ...
    def test_validate_bridge_smoke_expected_applied_mismatch_returns_nonzero(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            plan = root / "plan.json"
            bridge = root / "fake_bridge.py"
...
import sys
_ = json.loads(sys.stdin.read())
sys.stdout.write(json.dumps({"success": True, "severity": "info", "code": "BRIDGE_OK", "message": "ok", "data": {"applied": 1}, "diagnostics": []}))
""".strip(),
                encoding="utf-8",
            )
            exit_code, output = self.run_cli(
...
import sys
_ = json.loads(sys.stdin.read())
sys.stdout.write(json.dumps({"success": True, "severity": "info", "code": "BRIDGE_OK", "message": "ok", "data": {"applied": 2}, "diagnostics": []}))
""".strip(),
                encoding="utf-8",
            )
            exit_code, output = self.run_cli(
    ...
    def test_validate_bridge_smoke_expect_applied_from_plan_skips_expect_failure_case(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
...
parser.add_argument("--expected-code", default=None)
...
""".strip(),
            ...
            self.assertEqual("OK", summary["data"]["cases"][0]["expected_code"])
            self.assertEqual("OK", summary["data"]["cases"][0]["actual_code"])
            self.assertTrue(summary["data"]["cases"][0]["code_matches"])
    ...
    def test_validate_smoke_batch_expected_code_mismatch_returns_nonzero(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            plan = root / "avatar_plan.json"
            project = root / "avatar_project"
...
""".strip(),
        ...
        self.assertEqual("EXPECTED", case["expected_code"])
        self.assertEqual("OK", case["actual_code"])
        self.assertFalse(case["code_matches"])
        self.assertFalse(case["matched_expectation"])

    def test_validate_smoke_batch_rejects_empty_expected_code(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            plan = root / "avatar_plan.json"
            project = root / "avatar_project"
...
parser.add_argument("--out", required=True)
args, _ = parser.parse_known_args()
payload = {"success": True, "severity": "info", "code": "OK", "message": "ok", "data": {}, "diagnostics": []}
Path(args.out).write_text(json.dumps(payload), encoding="utf-8")
print(json.dumps(payload))
raise SystemExit(0)
""".strip(),
...
""",
    ...
    def test_report_smoke_history_fails_on_code_mismatch_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            summary = temp / "summary.json"
            out_csv = temp / "history.csv"
    ...
    def test_report_smoke_history_fails_on_observed_timeout_coverage_per_target_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            summary = temp / "summary.json"
            out_csv = temp / "history.csv"
    ...
    def test_report_smoke_history_fails_on_profile_timeout_coverage_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            summary = temp / "summary.json"
            out_csv = temp / "history.csv"
    ...
    def test_report_smoke_history_fails_on_profile_timeout_coverage_per_target_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp = Path(temp_dir)
            summary = temp / "summary.json"
            out_csv = temp / "history.csv"
