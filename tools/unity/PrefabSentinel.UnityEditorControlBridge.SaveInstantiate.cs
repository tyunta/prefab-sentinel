...
namespace PrefabSentinel
{
    ...
    public static partial class UnityEditorControlBridge
    {
        ...
        // Issue #193 — safe-save handler. Parses the protected-component
        // list, calls the underlying ``SaveAsPrefabAsset`` invocation
        // through the private ``SaveAsPrefabCore`` helper (which also
        // populates the console-classification snapshot fields), then
        // — when the parsed list has at least one entry — re-attaches
        // any protected type the save stripped via ``Undo.AddComponent``
        // and re-saves so the resulting asset matches the safe-save
        // contract. Issue #228: an explicitly empty parsed list is a
        // request for raw-save mode and bypasses the strip-and-reattach
        // pipeline; the response still reports orphan modifications and
        // an empty re-attached-component list. The
        // ``EDITOR_CTRL_SAFE_SAVE_PREFAB_PROTECT_REQUIRED`` envelope is
        // emitted only when the protected-list field is absent from the
        // request payload.
        private static EditorControlResponse HandleSafeSaveAsPrefab(EditorControlRequest request)
        {
            // Absent-field branch: the payload omitted the list entirely.
            // An explicitly empty list (``[]``) parses as a non-null
            // empty array and routes through the raw-save branch below.
            if (string.IsNullOrEmpty(request.protect_components_json))
                return BuildError(
                    "EDITOR_CTRL_SAFE_SAVE_PREFAB_PROTECT_REQUIRED",
                    "protect_components is required; pass an empty list ([]) to opt into raw-save mode.");
