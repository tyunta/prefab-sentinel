...
- Added Unity bridge smoke runner for end-to-end bridge invocation from patch plans:
  ...
  - expectation checks (`--expect-failure` / `--expected-applied` / `--expect-applied-from-plan`) and optional response export (`--out`)
...
- Added shared bridge smoke library + CLI command:
  ...
  - `prefab-sentinel validate bridge-smoke ...` (`--expect-failure` / `--expected-applied` / `--expect-applied-from-plan` / `--out` supported)
  - output `data` now includes apply assertion evidence (`expected_applied` / `expected_applied_source` / `actual_applied` / `applied_matches`) when apply assertion is enabled
