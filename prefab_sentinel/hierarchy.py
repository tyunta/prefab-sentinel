...
class HierarchyNode:
    ...
    # Issue #238: rect anchor data lifted from the underlying transform
    # record so a hierarchy serialiser can decide rect-vs-plain by node
    # field presence alone.  ``None`` on plain Transform nodes; never
    # populated on a non-RectTransform record.
...
def analyze_hierarchy(
    ...
    def _build_node(go_fid: str, depth: int) -> HierarchyNode:
        ...
        rect_anchor = t.rect_anchor if (t and t.is_rect_transform) else None
...
def format_tree(
    ...
    *,
    ...
    def _render(node: HierarchyNode, prefix: str, is_last: bool) -> None:
        ...
        # Issue #238: annotate the rect-specific anchor + size delta only
        # on RectTransform nodes; plain Transform nodes never carry the
        # annotation so a reader can tell rect-vs-plain from the rendered
        # string alone.
        if node.rect_anchor is not None:
            anchor = node.rect_anchor
            label += (
                f" [anchor=({anchor.anchor_min[0]},{anchor.anchor_min[1]})-"
                f"({anchor.anchor_max[0]},{anchor.anchor_max[1]}) "
                f"size={anchor.size_delta[0]}x{anchor.size_delta[1]}]"
            )
        if node.depth == 0:
            lines.append(label)
        else:
