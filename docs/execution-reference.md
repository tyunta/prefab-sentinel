...
The MCP child receives the explicit `--watch-dir` plus the current `UNITYTOOL_BRIDGE_INSTANCE_ID`, when supplied by the operator's Bridge connection info. The instance value is captured when the transport is constructed so the child can verify a no-op deployment against the same Editor. Unrelated parent environment settings are not forwarded. After an Editor process restart, update the instance ID before starting acceptance.

The secondary `editor_recompile` probe is omitted only when the pre-deploy target/running-version comparison matches and deployment confirms `promotion_state=already_current`. The report records `compile_observation=not_required`, an empty `secondary_recompile`, and no `recompile` phase. Environment, Console, smoke, cleanup, and postcondition checks still run. Real promotion or changed running-version evidence retains the secondary probe.
