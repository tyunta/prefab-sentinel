...
This module is the surviving member of the prior duplicate pair
(``test_class_naming_convention.py`` was deleted under issue #222 Phase
2 Necessity Check); it reads the un-mutated ``tests/`` tree to verify
a source-text invariant and shells out to ``ruff`` for the redundant
linter check.  The ``source_text_invariant`` marker excludes the
module from mutmut's pytest selection because mutating
``prefab_sentinel/`` cannot affect what this test observes.
"""
...
# Issue #167 / #222: source-text invariant — see module docstring.
