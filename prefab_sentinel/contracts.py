...
class ToolResponse:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        Issue #304: the diagnostics list crosses the MCP wire surface
        in a single unified four-key shape regardless of internal
        construction path. ``_diagnostic_to_wire`` is the
        normalization boundary so callers of ``to_dict`` never see the
        legacy ``Diagnostic`` field names on the wire.
        """
        return {
            ...
            "data": self.data,
            "diagnostics": [
                _diagnostic_to_wire(diag, default_severity=self.severity)
                for diag in self.diagnostics
            ],
        }


def _diagnostic_to_wire(
    diag: Diagnostic, default_severity: Severity,
) -> dict[str, Any]:
    """Normalize a ``Diagnostic`` instance to the unified MCP wire shape.

    Issue #304: every diagnostic that crosses the MCP wire — whether
    constructed via ``Diagnostic`` and routed through ``to_dict`` or
    built ad-hoc by a session-level emitter — surfaces in the
    four-key shape ``{severity, code, message, data}``. The legacy
    ``Diagnostic`` field names map to wire slots as follows:

    * ``detail``   → ``code`` (machine-readable category)
    * ``evidence`` → ``message`` (human-readable text; falls back to
      ``code`` when ``evidence`` is empty so consumers always observe
