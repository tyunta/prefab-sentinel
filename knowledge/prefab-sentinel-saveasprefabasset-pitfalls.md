---
tool: prefab-sentinel-saveasprefabasset-pitfalls
version_tested: "prefab-sentinel 0.5.167"
...
`PrefabUtility.SaveAsPrefabAsset` は単発で呼ぶと「黙ってコンポーネントを剥がす」「nested override が orphan 化する」「TextMeshPro の必須参照を消す」など、保存自体は成功 (`success == true`) なのに saved asset が壊れているケースがある。`editor_safe_save_prefab` MCP ツールは `protect_components` で守るべき型名を必須引数として受け取り、保存後に再アタッチと差分レポートを返すことでこの罠を吸収する。

このドキュメントは「`SaveAsPrefabAsset` を直接呼んではいけないケース」のカタログ。代替手段は **常に `editor_safe_save_prefab`** とその `protect_components` リスト。MCP 経由のワークフローでカバーできない `[MenuItem]` builder（Editor から直接 `LoadPrefabContents` → `SaveAsPrefabAsset` を回す形）向けのフォールバックパターンも併せて記載する。
...
実例: `Ui.prefab` の root 直下に空の `Inner` GameObject を挿入し、既存子要素を
`Inner` 配下へ reparent して save。次に `Host.prefab` (Ui.prefab を nest)
を **任意のタイミングで save** すると、PrefabInstance.m_Modifications にあった以下の
override が **配列ごと丸ごと消える**:

- `SomeButton.m_OnClick.m_PersistentCalls.m_Calls.Array.size`
- `SomeButton.m_OnClick.m_PersistentCalls.m_Calls.Array.data[0].m_MethodName`
- `SomeButton.m_OnClick.m_PersistentCalls.m_Calls.Array.data[0].m_StringArgument`
- 他 5 button × ~6 entry = 約 40 modification

reparent された子要素 (`SomeButton` 等) の fileID は変わらないので override の `target.fileID`
...
private static void Wire() {
    var host = PrefabUtility.LoadPrefabContents(HostPrefabPath);
    try {
        var ui = FindByName(host.transform, "Ui");
        var ctrlUb = UdonSharpEditorUtility.GetBackingUdonBehaviour(
            host.GetComponentInChildren<Controller>());

        // 全配線を毎回ここに列挙する（冪等に再設定する）
        WireButtonByPath(ui, "Inner/ButtonA",        ctrlUb, "_OnButtonA");
        WireButtonByPath(ui, "Inner/Group/ButtonB",  ctrlUb, "_OnButtonB");
        WireFloatEvent(ui, "Inner/Group/SomeSlider", ctrlUb, "OnSliderChanged");
        WireBoolEvent (ui, "Inner/Group/SomeToggle", ctrlUb, "OnToggled");

        PrefabUtility.SaveAsPrefabAsset(host, HostPrefabPath);
...
### Mode 3 follow-up: TMP 3D variant (`TextMeshPro` / MeshRenderer 系)

uGUI 用の `TextMeshProUGUI` と並んで、3D MeshRenderer 系の **`TextMeshPro`**（namespace `TMPro`、Inspector の Add Component → Mesh → "TextMeshPro - Text"）にも同じ Mode 3 strip が再現する。コンポーネント型名は `TextMeshProUGUI` ではなく `TextMeshPro` で、両者は別 component。検証 3 項目を以下のとおり記録する。

| 検証項目 | 観察結果 | 取り扱い |
|---|---|---|
| (a) `m_fontAsset` が `SaveAsPrefabAsset` 後に null PPtr になるか | 3D 版 `TextMeshPro` でも uGUI 版と同じく nulled される。build 直後に `font` を明示代入する予防策が効くことを再現確認済み | 保存後 `editor_get_serialized_property` で `m_fontAsset` を読んで null PPtr (`{fileID: 0}`) なら明示再代入する |
| (b) `editor_safe_save_prefab` の `protect_components` が `["TextMeshPro"]` をカバーするか | safe-save handler の再アタッチ pipeline は名前比較ベース（型名文字列の完全一致）なので **`"TextMeshPro"`（uGUI ではなく 3D 版）を別エントリとして指定する必要がある**。uGUI 用に `["TextMeshProUGUI"]` だけ指定しても 3D 版はカバーされない | 3D + uGUI を同一 prefab に混在させる場合は `protect_components=["TextMeshPro", "TextMeshProUGUI"]` の両方を渡す |
| (c) 3D variant に同じ `LiberationSans SDF.asset` を割当てて良いか | TMP の `TMP_FontAsset` は uGUI / 3D 共通の asset 型なので、`Assets/TextMesh Pro/Resources/Fonts & Materials/LiberationSans SDF.asset` を 3D 変種にも代入できる。3D 専用の別 default font asset は TMP package には用意されていない（同一 asset を共有する設計） | 3D 変種でも `tmp.font = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(LiberationSansSdfPath)` で OK。fontSize は 3D ワールド単位なので uGUI と数値比較せず、Scene 内で個別調整する |

3D 版の builder MenuItem パターン（Mode 3 の uGUI コードを 3D 用に置換した形）:
...
// AddComponent で 3D 版 TMPro コンポーネントを追加（TextMeshProUGUI ではない）
var tmp = go.AddComponent<TMPro.TextMeshPro>();
tmp.font = fontAsset;
tmp.text = "Foo";
tmp.fontSize = 1.0f;  // 3D ワールド単位、uGUI とは数値桁が違うので注意
```

`editor_create_ui_element` の MCP ラッパは `TextMeshProUGUI` のみを生成対象にしており、
3D 版は生成 path をサポートしていない（issue #195 の uGUI スコープ）。3D 版を扱う場合は
上記 builder MenuItem パターンか、`editor_run_script` で `AddComponent<TMPro.TextMeshPro>`
+ `tmp.font` 明示代入を行う。`editor_safe_save_prefab` 経由なら `protect_components` に
`"TextMeshPro"` を入れて strip-and-reattach pipeline で吸収する。
...
var hint = (HintController)UdonSharpEditorUtility.AddUdonSharpComponent(
    controllerGo, typeof(HintController));

// ✓ extension method
using UdonSharpEditor;
var hint = controllerGo.AddUdonSharpComponent<HintController>();
```
