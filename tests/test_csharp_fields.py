...
class TestParseSerializedFields(unittest.TestCase):
    ...
    def test_public_field_is_recognized_as_serialized_with_full_field_shape(
        self,
    ) -> None:
        fields = parse_serialized_fields("public float speed;")

        # Pin (name, type_name, is_serialized, is_public) as a single
        # tuple so a mutation that, e.g., flips is_public while leaving
        # is_serialized true is named in the same failure message.
        self.assertEqual(1, len(fields))
        f = fields[0]
        self.assertEqual(
            ("speed", "float", True, True),
            (f.name, f.type_name, f.is_serialized, f.is_public),
        )
    ...
    def test_private_field_not_serialized(self) -> None:
        ...
        self.assertEqual(1, len(fields))
        self.assertFalse(
            fields[0].is_serialized,
            msg="private field without [SerializeField] must not be serialised",
        )
    ...
    def test_serialize_field_attribute(self) -> None:
        ...
        self.assertTrue(
            fields[0].is_serialized,
            msg="[SerializeField] on private field must mark it serialised",
        )
        self.assertFalse(
            fields[0].is_public,
            msg="private field must remain non-public even when serialised",
        )
    ...
    def test_serialize_field_separate_line(self) -> None:
        ...
        self.assertTrue(
            fields[0].is_serialized,
            msg="[SerializeField] on prior line must still mark field serialised",
        )
    ...
    def test_nonserialized_public_field(self) -> None:
        ...
        self.assertFalse(
            fields[0].is_serialized,
            msg="[NonSerialized] on public field must override public-equals-serialised rule",
        )
    ...
    def test_header_attribute_token_is_captured_verbatim(self) -> None:
        # The contract is the EXACT attribute token, not "contains
        # 'Header'": a parser regression that drops the argument string
        # would slip through an `any("Header" in a)` predicate.
        source = '[Header("Movement")] public float runSpeed;'

        fields = parse_serialized_fields(source)

        self.assertEqual(1, len(fields))
        self.assertIn('Header("Movement")', fields[0].attributes)

    def test_tooltip_attribute_token_is_captured_verbatim(self) -> None:
        source = '[Tooltip("Speed in m/s")] public float speed;'

        fields = parse_serialized_fields(source)

        self.assertEqual(1, len(fields))
        self.assertIn('Tooltip("Speed in m/s")', fields[0].attributes)

    def test_range_attribute_token_is_captured_verbatim(self) -> None:
        source = "[Range(0, 100)] public int value;"

        fields = parse_serialized_fields(source)

        self.assertEqual(1, len(fields))
        self.assertIn("Range(0, 100)", fields[0].attributes)

    def test_two_stacked_attributes_are_captured_as_exact_set(self) -> None:
        # Pin exact membership: the documented contract is "two attributes,
        # both verbatim".  ``assertGreaterEqual(len, 2)`` would silently
        # accept a parser regression that drops one and duplicates another.
        ...
        self.assertEqual(
            {'Header("X")', "Range(0, 1)"},
            set(fields[0].attributes),
        )

    def test_formerly_serialized_as_attribute_token_is_captured_verbatim(
        self,
    ) -> None:
        ...
        self.assertEqual(1, len(fields))
        self.assertIn(
            'FormerlySerializedAs("oldSpeed")', fields[0].attributes
        )
...
"""
    ...
    def test_protected_field_not_serialized_without_attribute(self) -> None:
        ...
        self.assertFalse(
            fields[0].is_serialized,
            msg="protected field without [SerializeField] must not be serialised",
        )
    ...
    def test_protected_field_serialized_with_attribute(self) -> None:
        ...
        self.assertTrue(
            fields[0].is_serialized,
            msg="[SerializeField] on protected field must mark it serialised",
        )
    ...
    def test_to_dict(self) -> None:
        ...
        self.assertTrue(
            d["is_serialized"],
            msg="to_dict() must echo is_serialized=True from the source field",
        )
...
class TestParseClassInfo(unittest.TestCase):
    ...
    def test_simple_class_header_pins_name_base_class_and_field(self) -> None:
        ...
        self.assertEqual(
            ("PlayerScript", "MonoBehaviour", 1, "speed"),
            (info.name, info.base_class, len(info.fields), info.fields[0].name),
        )
...
class TestResolveInheritedFieldsDiagnostics(unittest.TestCase):
    ...
    def test_undecodable_parent_records_one_diagnostic(self) -> None:
        ...
        # Sink count is non-deterministic (1 when only the build phase
        # surfaces, 2 when the per-file phase also surfaces) so the
        # count is not pinned exactly; instead pin membership of the
        # documented detail in the materialised detail list so a
        # mutation that drops or renames the diagnostic is named in
        # one failure message.
        details = [d.detail for d in sink]
        self.assertIn("unreadable_file", details, msg=f"sink={sink!r}")
...
class TestFindDerivedGuidsDiagnostics(unittest.TestCase):
    ...
    def test_undecodable_file_records_one_diagnostic(self) -> None:
        ...
        # The count is non-deterministic across phases so pin
        # membership in the materialised detail list: a mutation that
        # drops or renames the documented diagnostic is named in one
        # failure message.
        details = [d.detail for d in sink]
        self.assertIn("unreadable_file", details, msg=f"sink={sink!r}")
