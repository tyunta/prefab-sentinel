---
tool: prefab-sentinel-editor-camera
...
MCP ツール `editor_set_camera` / `editor_get_camera` / `editor_frame` の挙動カタログ。対象は Unity SceneView カメラ（Game ビューではない）。
...
## ROI screenshot と scene-view refresh primitive (issue #249 / #242)

### `editor_screenshot` の `crop_roi` 引数
- 既定は空文字列で、フルフレーム撮影。
- 4 つの preset 名のいずれか: `eye_left` / `eye_right` / `mouth` / `auto_face`。
  - preset を指定すると、bridge 側は scene-view camera を該当領域に framing してから render し、`SceneView.LookAt(..., instant: true)` で **撮影前の pivot/rotation/size/orthographic** を復元する。
  - 復元のため、preset 撮影前後で scene-view カメラ状態は外見上変化しない（Editor の visual 状態を汚さない）。
- pixel quadruple 形式: `"x,y,w,h"`（カンマ区切りの非負整数 4 つ）を渡すと、現在の scene-view camera state で render したあと、PNG をその矩形でクロップする。pixel rect path では camera framing は変えない。
- 不正値（allowlist でも 4-tuple でもない文字列）は wrapper 側で `CROP_ROI_INVALID` の error envelope を返して bridge を呼ばない。
- 成功 envelope には:
  - `crop_roi_applied`: preset 名（4 つのいずれか）または `pixel_rect` 文字列。
  - `crop_bounds`: `{x, y, w, h}` の int オブジェクト（pixel rect path で実際にクロップした座標）。

### `editor_force_scene_view_refresh()`
- bridge action `force_scene_view_refresh` を 1 round-trip で叩く。
- 内部で全 SkinnedMeshRenderer の `forceMatrixRecalculationPerRender = true` を立て、`EditorApplication.QueuePlayerLoopUpdate()` を呼ぶ。
- 用途: 多数の camera render を screenshot 経路の外から行うときに、毎 render ごとに bridge round-trip するのを避ける。1 回だけ refresh primitive を呼んで以降の render はクライアント側の C# / menu invocation でまわす。
- レスポンスの `renderers_touched` に touch 件数が入る（0 件でも success）。
- 例外発生時は `EDITOR_CTRL_FORCE_REFRESH_FAILED`。
