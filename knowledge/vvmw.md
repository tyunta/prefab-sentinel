...
> VVMW Core を独自 UI から制御する構成で得た知見。実環境 + ClientSim + multi-PC で検証済み。
...
### `Core.synced = false` の autoplay 強制

**症状**: `Core.synced = false` (Inspector で OFF / prefab で `synced: 0`) の構成で `Core.PlayUrl` した直後、listener 経由で `Core.Pause()` を呼んでも **直後に強制 Play される**。「再生制御を local に閉じたつもりでもプレイヤー上で動画が動き始める」症状の根本原因。
...
`Core.Pause()` 等は内部で `RequestSync()` を呼ぶが、`synced=false` だと `RequestSerialization` は走らず early-return する (= 帯域消費なし)。設計通り「再生制御は完全 local」を意図した構成。**video 同期 (URL/baseline) を別 UB (SyncBridge 等) で済ませる** 構成で有効。
...
└── Ui/                             # 独自 UI (再生制御 / Volume 等のコントロール)
    └── LocalPlayerController  # VVMW Core を listener 登録、Pause 等を呼ぶ
```
