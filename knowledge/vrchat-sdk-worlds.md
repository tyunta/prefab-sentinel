...
| PipelineManager | `VRC.Core` | VRCSDK3.dll (GUID: `4ecd63eff847044b68db9453ce219299`) — VRCWorld に付随 |
...
## L2: WorldSpace Canvas 構築（VRC_UiShape 必須）

> 出典: <https://creators.vrchat.com/worlds/components/vrc_uishape> および UI Events <https://creators.vrchat.com/worlds/udon/networking/network-components/vrc_ui_shape>。

WorldSpace Canvas を VRChat ワールド内のインタラクティブ UI として機能させるには、以下のハードルールを **必ず** 守る。1 つでも外れるとレーザー入力が通らず UI として機能しない、または衝突判定がワールド全体を覆って他オブジェクトのレーザーをブロックする。

### Canvas 必須プロパティ

| プロパティ | 値 | 根拠 |
|-----------|----|------|
| `Canvas.renderMode` | `WorldSpace` | ScreenSpaceOverlay / Camera は VRChat ワールドでは表示されない |
| `Transform.localScale` | `(0.01, 0.01, 0.01)` | ピクセル単位の `sizeDelta` をメートル単位の世界座標へ変換するための共通スケール（1 px = 0.01 m） |
| `RectTransform.sizeDelta` | ピクセル単位（例 `1024 x 768`） | `localScale = 0.01` 前提でメートル単位に換算される |
| `VRC_UiShape` | Canvas と同じ GameObject に配置 | Canvas 単体ではレーザー入力を受け付けない（VRC_UiShape が衝突判定を生成） |

### BoxCollider Z 軸の罠（最重要）

VRC_UiShape は内部で `BoxCollider` を自動付与・自動リサイズする。この BoxCollider の `size` は **Canvas のローカル単位**（`localScale = 0.01` 前提）で設定される:

- Canvas の `localScale = (0.01, 0.01, 0.01)` のとき、BoxCollider Z = `1.0` は **0.01 m = 1 cm** の薄さ
- Canvas の `localScale = (1, 1, 1)` のままだと、同じ Z = `1.0` が **1 m** の厚さになり、Canvas の前後 1 m 全部が衝突判定になる

`localScale = 1` で配置した WorldSpace Canvas はこの 1 m 厚コライダーで **他オブジェクトのレーザー入力を遮断** する。Canvas 自身の UI 入力も、レーザー先端が UI 平面より奥側に当たることでクリック判定が通らなくなり機能しない。

### 修正手順

1. Canvas を含む GameObject の `Transform.localScale` を `(0.01, 0.01, 0.01)` に設定
2. 同じ GameObject に `VRC_UiShape` を Add Component（自動的に BoxCollider が付き、リサイズされる）
3. UI 要素のサイズはピクセル単位で `RectTransform.sizeDelta` に指定（`localScale` 0.01 倍されてメートルに換算される）
4. BoxCollider の `size` は VRC_UiShape のリサイズに任せる。手動で 1.0 m 厚に戻さない

### Inspector 表示と SerializedProperty の差異

- Canvas の `Render Mode` インスペクター表示 → SerializedProperty: `m_RenderMode`
- VRC_UiShape の BoxCollider はランタイム時に付与されるため、Prefab YAML 上で必ずしも見えない（プレハブ保存タイミングに依存）
...
- VRCWorld の SceneDescriptor は `guid:661092b4` で確認（VRCSDK3.dll）
- PipelineManager は `guid:4ecd63ef` で VRCWorld に付随
- UdonBehaviour (`guid:45115577`) が VRCWorld にも存在（SDK 内部使用）
- VRCSceneDescriptor の `spawns` フィールドは Transform 参照で `find_unity_symbol` から読み取り可能
- `validate_refs` でワールドシーンの参照整合性を検証可能
