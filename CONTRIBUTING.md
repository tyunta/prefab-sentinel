...
The HTTP endpoint is fixed to `http://127.0.0.1:<port>/mcp`; there is no public-host option. Both transports expose a Tools-only surface, but stdio supports modern and supported legacy clients while Streamable HTTP is modern-only. Use [the execution reference's transport/version table](./docs/execution-reference.md#実行方法) for the accepted revisions, request methods, notifications, and lifecycle requirements; this entry point does not maintain a separate compatibility matrix.
