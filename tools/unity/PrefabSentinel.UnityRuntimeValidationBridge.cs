...
namespace PrefabSentinel
{
    ...
    public static class UnityRuntimeValidationBridge
    {
        ...
        internal static void WriteResponse(string responsePath, RuntimeResponse response)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Debug.LogWarning($"[PrefabSentinel] WriteResponse: {fallbackEx.GetType().Name}: {fallbackEx.Message}");
                }
            }
        }
    ...
    internal sealed class RuntimeValidationRunner : MonoBehaviour
    {
        ...
        {
            // The Editor Bridge keeps the editor process alive across requests;
            // write the response file and return control to the player loop.
