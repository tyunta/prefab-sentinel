...
def editor_batch_set_blend_shape(
    ...
    Args:
        hierarchy_path: Hierarchy path to the GameObject with a
            SkinnedMeshRenderer.
        shapes: List of ``{"name": str, "weight": float}`` entries.
...
def register_editor_batch_tools(server: FastMCP) -> None:
    ...
    # single module-level callable; register it directly under its
    # external name so the MCP tool and the module-level import share
    # one implementation.
    server.tool(name="editor_batch_set_blend_shape")(editor_batch_set_blend_shape)
