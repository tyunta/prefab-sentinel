...
class PostconditionSchemaSuccessRowsTests(unittest.TestCase):
    ...
    def test_asset_exists_by_resource_returns_post_schema_ok(self) -> None:
        ...
        # Pin the documented success envelope as a single tuple so a
        # mutation that swaps any one of (success, code, severity, data)
        # is named in the same failure message.
        self.assertEqual(
            (
                True,
                "POST_SCHEMA_OK",
            ...
            (response.success, response.code, response.severity, response.data),
        )
