...
file-read failure と read 後の schema failure は transport ごとに切り分ける。response file 自体を symlink ではない regular file・16 MiB 以下・UTF-8 standard JSON かつ host `float` 変換後も有限な値として受理できなければ、editor-control は `EDITOR_BRIDGE_RESPONSE_READ`、runtime は `RUN_EDITOR_BRIDGE_RESPONSE` を返す。editor-control の private log は response path と exception traceback を保持するが、runtime log は固定文 `Runtime Editor Bridge response read failed` だけを記録し、caught filesystem / decode / close detail を保持しない。read 後に共通6フィールドまたは `success=true` + `severity=error|critical` 禁止則を満たさなければ、editor-control は `EDITOR_BRIDGE_RESPONSE_SCHEMA`、runtime は `RUN_PROTOCOL_ERROR` を返す。runtime の operation payload 違反も `RUN_PROTOCOL_ERROR` である。editor-control の共通 shape を通過した後は、各 consumer が `EDITOR_REFLECT_RESPONSE_SCHEMA`、`ASSET_DELETE_RESPONSE_SCHEMA`、`WRITE_RESPONSE_SCHEMA` のような operation payload code を所有する。valid な `success=false` bridge response は schema error や成功に変換せず、元 operation の failure として読む。

Bridge deploy は Issue #274 の固定 `phase` で最初の failure boundary を既存の private Python host logging / stderr へ emit する。`manifest_directory_read` / `manifest_file_read` は builtin `OSError` type 名（それ以外は `OSError`）と `errno` だけを持ち、filename、path、例外本文、traceback を持たない。`post_promotion_verification` は固定 verification code、allowlist 済み first-action code（`DEPLOY_OK` / `DEPLOY_REFRESH_FAILED`、その他は `DEPLOY_OTHER`）、正規化済み promotion state、target-complete boolean だけを recovery 前に残す。`transaction_cleanup` は最初に失敗した `rmtree` operation を `unlink` / `child_rmdir` / `root_rmdir` / `scandir` / `open` / `lstat` / `close` / `other`（callback 前の失敗は `rmtree`）へ固定分類し、builtin `OSError` type 名（それ以外は `OSError`）と exact-int の `errno` / `winerror`（非整数または欠落は `None`）だけを残す。callback は同じ例外を直ちに再送出するため、削除の retry や継続は行わない。public response / code / schema / return identity は変えず、raw private response message や payload を転記しない。これらの producer-side emission は Python host process の logging lifetime にだけ属し、通常の acceptance CLI 終了後の durable retention を保証しない。operator が保存先を選ぶ private diagnostic storage は Issue #259 の別課題である。

### Patch writer boundary の再検査

`set_material_property` / `copy_asset` / `rename_asset` / `delete_asset` / `delete_assets` / `revert_overrides` が `PATCH_WRITER_BOUNDARY_FAILED` を返したら、最初に `data.mutation_state` と `data.cache_state` を読む。`not_started / unchanged` は mutation 前に停止した状態、`unknown / invalidated` は confirmed dispatch 後に有効な response を受け取れず、適用成否を断定できない状態である。後者、または成功応答に `PATCH_WRITER_REINSPECTION_REQUIRED` が付いた場合は、同じ asset へ再度 write する前に対象toolのread/dry-runをやり直す。`applied / invalidated` は mutation成功後のrefreshだけが失敗した状態なので、元の `success=true` を失敗へ読み替えず、再inspectionでEditor側の観測を更新する。`cache_state="unknown"` はcache invalidation自体も失敗した状態であり、process内cacheを信頼せず新sessionでreadをやり直す。raw exceptionはpublic responseではなくPython hostのprivate logで確認する。

`get_project_status` の watch-dir 診断は `data.bridge.connection_state`, `data.bridge.code`, `data.bridge.blocker_class`, `data.bridge.suggested_next_action` と `data.blockers[]` から読む。`connection_state="misconfigured"` と `EDITOR_BRIDGE_WATCH_DIR_MISMATCH` は、Codex host と Unity Editor Bridge が同じ watch directory を使っていないことを fresh private identity status が証明した状態である。両者の `UNITYTOOL_BRIDGE_WATCH_DIR` を同じ directory に設定してから再実行する。自動探索・自動再設定はしない。host filesystem pathやprobe例外本文はpublic statusに出ない。marker IDs、watch paths、timestamps、private status content、raw exceptions are not public。詳細が必要な場合はPython hostのprivate logで `Editor Bridge watch directory status probe failed for ...`, `Editor Bridge request write failed for ...`, `Editor Bridge response status probe failed for ...`, `Editor Bridge response read failed for ...` と直後のtracebackを確認する。Bridge由来diagnostic/failureのraw message/path/evidenceもprivate `mcp_tools_session` logにだけ残る。session identityの `project_root` とlive `actual_project_root` は別の誤接続診断であり、watch-dir redaction後も公開される。
