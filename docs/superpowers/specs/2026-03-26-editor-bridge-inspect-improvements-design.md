...
refresh の `send_action` 呼び出しは fire-and-forget とする。失敗してもスクリーンショット撮影は続行する（refresh はベストエフォートの前処理であり、失敗で撮影を中断する意味がない）。refresh 失敗時は `logger.warning` で記録する。MCP レスポンスには含めない。

デフォルト True の根拠: refresh 忘れによるスクショ不整合が実運用で頻発するため。不要な場合のみ `refresh=False` で回避する。

### B. Nested Prefab シンボルツリー展開

`SymbolTree.build()` にキーワード専用パラメータを追加。更新後のシグネチャ:
...
def build(
    ...
    *,
    ...
    _depth: int = 0,  # 内部用: 再帰深度カウンタ（MCP 非公開）
) -> SymbolTree:
```

`_depth` は再帰呼び出し時にインクリメントし、`_MAX_NESTED_DEPTH` 以上なら展開をスキップする。外部からの呼び出しでは省略する（デフォルト 0）。
...
5. 再帰深度上限: 10（`_MAX_NESTED_DEPTH` 定数。Unity の Nested Prefab 実用上限に合わせた防御的上限）。`_depth` パラメータで追跡し、`_depth >= _MAX_NESTED_DEPTH` なら展開スキップ
...
`resolve()` での PrefabInstance ノードは透過的コンテナとして機能する。

具体的には `_resolve_segments()` の各ステップで、`candidates` リスト内の `kind == PREFAB_INSTANCE` ノードをセグメントマッチ前に展開（フラット化）する。PrefabInstance ノード自体は候補から除外し、その `children` を同じ候補リストに差し替える。これにより PrefabInstance ノードはパスセグメントを消費せず、子ノード群が直接の候補として扱われる。

```python
# _resolve_segments 内の候補展開（疑似コード）
flat_candidates = []
...
# flat_candidates に対してセグメントマッチを実行
```
...
`to_dict(depth_limit)` において PREFAB_INSTANCE ノードは通常のノードと同様に 1 深度を消費する。特別扱いしない。

#### fileID インデックスの扱い

PrefabInstance マーカーノード自体は `_file_id_index` に登録する（親ファイル内の一意な fileID を持つため）。

子 Prefab から展開されたノード群は `_file_id_index` に登録 **しない**。子 Prefab のノードは別ファイルの fileID 名前空間に属するため、親の `_file_id_index` に混在させると衝突が起きる。`resolve_file_id()` は単一ファイルスコープの操作であり、Nested 展開先のノードには使用しない。

#### セッションキャッシュ

`session.py` の `SymbolTree` キャッシュは `(path, mtime, include_properties)` をキーとしている。`expand_nested=True` の場合はキャッシュをバイパスし、常に新規ビルドする。理由: 展開済みツリーは子 Prefab ファイルの内容にも依存するため、親ファイルの mtime だけでは無効化判定ができない。頻度の低いリクエストなのでキャッシュ不在のコストは無視できる。

`session.py` の `get_symbol_tree()` メソッドに `expand_nested` パラメータを追加し、True の場合はキャッシュ読み書きをスキップして `SymbolTree.build()` を直接呼ぶ。
...
具体的には `_validate_scene_add_component_op` 内の `_require_handle_ref` 呼び出しで:
...
`_inspect_variant_materials` の既存フローの後に Nested Prefab フォールバックを追加する。挿入位置は既存の `_build_stripped_renderer_materials` フォールバック（`renderers` が空の場合に stripped ブロックから復元する処理）の **後**:

1. 既存フロー: (a) base Prefab の non-stripped renderer ブロックから収集、(b) 空なら `_build_stripped_renderer_materials` で stripped ブロックから復元。**両方が空を返した場合にのみ**、以下の Nested 展開を第 3 フォールバックとして試みる
...
4. `_inspect_base_materials` が返した `RendererMaterials` リストに対し、呼び出し後に `renderer.source_prefab = child_prefab_rel_path` で後付けする（`_inspect_base_materials` のシグネチャは変更しない）
...
`diagnostics` は `list[str]` で、人間可読な理由文を格納する（例: `"No renderer blocks found in base prefab or nested prefabs"`）。MCP ツール `inspect_materials` のレスポンス JSON には `diagnostics` キーとして含める。空の場合はキーを省略する（後方互換のため）。

`format_materials()` は `diagnostics` が非空の場合、renderer ブロックの後に `[diagnostic] {message}` 形式で各行を追記する。
...
| `prefab_sentinel/session.py` | `get_symbol_tree()` に `expand_nested` パラメータ追加、True 時キャッシュバイパス |
| `prefab_sentinel/orchestrator.py` | `get_unity_symbols` に `guid_to_asset_path` 受け渡し、`inspect_materials` の renderer dict に `source_prefab` 追加・`data` に `diagnostics` 追加（空時省略） |
...
- **A**: delegation テストで `refresh=True` 時に `send_action` が 2 回呼ばれ、1 回目が `refresh_asset_database`、2 回目が `capture_screenshot` であることを検証。`refresh=False` 時は `capture_screenshot` の 1 回のみ。
