...
- `editor_get_bounds(hierarchy_path, source="auto", include_children=True)` は renderer / collider / RectTransform contributors を world-space AABB に集約し、center / extents / size / min / max / contributor metadata を返す。contributors が無い場合は `EDITOR_CTRL_BOUNDS_UNAVAILABLE`。
- `editor_measure_distance(hierarchy_path, target_path, mode="pivot", bounds_source="auto")` は pivot 距離、bounds center 距離、または AABB surface 距離を返す。`surface` では overlap 距離は 0。
