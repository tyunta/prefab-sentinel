---
tool: prefab-sentinel-build-from-scratch
...
UI prefab を Builder script + MCP ツールでゼロから組む案件で踏んだ pitfalls と、回避できる現実的なパターンの記録。
...
B. **Build と Wire を分離** + Wire 段階で AddUdonSharpComponent。Build は visuals のみ
   保存し、Wire で Host.prefab に `Foo.prefab` の instance を nest した後、
   instance に追加する。

C. **専用 U# behaviour を新規追加せず、既存 U# behaviour を拡張する**。
...
3. **Wire** — `Host.prefab` を `LoadPrefabContents` で開いて、
...
```bash
# Wire 結果検証
grep -c "<wired-component-guid>" Host.prefab  # PrefabInstance count
grep -E "value: (_On|On)[A-Z]" Host.prefab | sort | uniq -c  # listener event names
grep -B1 -A2 "propertyPath: controller" Host.prefab  # field override 確認
```
